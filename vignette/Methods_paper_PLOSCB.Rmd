...
require(CTD)
require(huge)
for (model in c("cit", "gamt", "msud", "mma", "pa", "pku")) {
  for (fold in 1:length(cohorts[[model]])) {
    refs = data_mx[,which(colnames(data_mx) %in% diagnoses$id[which(diagnoses$diagnosis=="No biochemical genetic diagnosis")])]
    # ref_fill = apply(refs, 1, function(i) sum(is.na(i))/length(i)) #get rate of being detected by row in all reference samples; not needed in this dataset becasue highly-represented  metabolites are seletec beforehand
    print(sprintf("Learning graphs for diag %s...", model))
    diag_pts = cohorts[[model]]
    fill.rate = 1-(Miller2015$`Times identifed in all 200 samples`/200)
    ref_pts = colnames(refs)
    diag_data = data_mx[which(fill.rate<0.50), which(colnames(data_mx) %in% diag_pts)]#create patients matrix
    refs2 = refs[which(rownames(refs) %in% rownames(diag_data)), ]#create new reference matrix where only nodes highly- represented in patient matrix are included
    ...
    rm(diag_data, ref_data, diagnoses, inv_covmat, inv_covmatt, inv_covmatt_select,
       Miller2015, refs, refs2, diag_pts, fill.rate, ind)
    save.image(sprintf("Clinical_paper/graphs/bg_%s_ind_fold%d.RData", model, fold))
    rm(ig, ig_ref)
  }
}
```
...
``` {r pre-compute_single-node_rankings}
...
## Figure 7: Illustration of CTD's specificity.
...
data("Miller2015")
Miller2015 = Miller2015[,grep("IEM_", colnames(Miller2015))]
Miller2015 = Miller2015[, which(colnames(Miller2015) %in% 
                                diagnoses$id[which(diagnoses$diagnosis %in% 
                                                     c("Citrullinemia", "Guanidinoacetate methyltransferase", 
                                                       "Maple syrup urine disease", "Methylmalonic aciduria", 
                                                       "Phenylketonuria", "Propionic aciduria", 
                                                       "No biochemical genetic diagnosis"))])]
p0=0.1
p1=0.9
thresholdDiff=0.01
kmx=15
...
# Figure 7: Plot patient ID vs. bits for each disease-specific network
rm(list=ls())
setwd("/Users/lillian.rosa/OneDrive/MacFiles/9thCommitteeMeeting/")
require(ggplot2)
require(plotly)
require(pROC)
for (model in c("gamt", "pku", "pa", "mma", "msud", "cit")) {
  for (type in c("ind", "noLatent", "noPruning")) {
    ...
    for (fold in 1:length(cohorts[[model]])) {
      load(sprintf("Clinical_paper/loocv/loocv_%s_runCTD/model_%s_%s_fold%d_kmx%d.RData", type, model, type, fold, kmx))
      pts = unique(df$ptID)
    ...
    df_all = df_all[which(df_all$diag %in% c("cit", "gamt", "mma", "pa", "pku", "msud", "reference")),]
    ...
    dff = dff[which(dff$diag %in% c("gamt", "pku", "pa", "mma", "cit", "msud", "reference")),]
    p3= ggplot(dff, aes(x=diag, y=bits, fill=diag)) + geom_boxplot(size=2) + geom_hline(yintercept=-log2(0.05)) +  
      theme(text = element_text(size=20)) + ggtitle(sprintf("%s (%s)", model, type))
    orca(p3, sprintf("Clinical_paper/loocv/loocv_%s_runCTD/sn_%s_bg_methods_%s_loocv.png", type, model, type), width = 1000, height = 500)
...
for (model in c("cit", "msud", "mma", "pa", "pku", "gamt", "otc")) {
  ...
  ctd_module_select[[model]] = names(table(mdst)[table(mdst)>(length(cohorts[[model]])/3)])
...
kmx=15
data(Miller2015)
#data_mx = as.matrix(Miller2015[,which(colnames(Miller2015) %in% diagnoses$id[which(diagnoses$diagnosis %in% c("Citrullinemia", "Guanidinoacetate methyltransferase", "Maple syrup urine disease", "Methylmalonic aciduria", "Propionic aciduria", "Phenylketonuria", "No biochemical genetic diagnosis"))])])
data_mx = as.matrix(Miller2015[,grep("IEM_", colnames(Miller2015))])
fill.rate = 1-(Miller2015$`Times identifed in all 200 samples`/200)
data_mx = data_mx[which(fill.rate<0.20), ]
data_mx = data_mx[-grep("x - ", rownames(data_mx)),]
ref_data = data_mx[,which(colnames(data_mx) %in% diagnoses$id[which(diagnoses$diagnosis=="No biochemical genetic diagnosis")])]
...
for (model in c("cit", "msud", "mma", "pa", "pku")) {
  ...
  df_all = df_all[which(df_all$ptID %in% colnames(Miller2015)),]
  #df_all = df_all[which(df_all$diag %in% c("cit", "mma", "pa", "pku", "msud", "reference")),]
  df_all$bits = -log2(p.adjust(2^-(df_all$bits), method="fdr"))
  # Visualize LOOCV signal and compare to off-target diseased test patients
  ...
  df_mn = apply(data_mx[,which(colnames(data_mx) %in% cohorts[[model]])], 1, function(i) mean(na.omit(i)))
  df_mn = df_mn[which(abs(df_mn)>2)]
  df2 = as.data.frame(data_mx[which(rownames(data_mx) %in% names(df_mn)), which(colnames(data_mx) %in% df_all$ptID)])
  # Get diagnostic labels
  diags = colnames(df2)
  diags[-which(diags %in% cohorts[[model]])] = 0
  diags[which(diags %in% cohorts[[model]])] = 1
  df2 = rbind(df_all$bits, df2)
  ...
  df2_ctd = data_mx[which(rownames(data_mx) %in% ctd_module_select[[model]]), which(colnames(data_mx) %in% df_all$ptID)]
  df2_ctd = rbind(df_all$bits, df2_ctd)
  ...
  df2_infomap = data_mx[which(rownames(data_mx) %in% infomap_module_select[[model]]), which(colnames(data_mx) %in% df_all$ptID)]
  df2_infomap = rbind(df_all$bits, df2_infomap)
  rownames(df2_infomap)[1] = "metabolomics_evidence"
  df2_infomap = rbind(diags, df2_infomap)
  rownames(df2_infomap)[1] = "diag"
  df2_infomap = apply(df2_infomap, c(1,2), as.numeric)
  
  # PLS model w/ GMO feature selection
  df2_gmo = data_mx[which(rownames(data_mx) %in% gmo_module_select[[model]]), which(colnames(data_mx) %in% df_all$ptID)]
  df2_gmo = rbind(df_all$bits, df2_gmo)
  rownames(df2_gmo)[1] = "metabolomics_evidence"
  df2_gmo = rbind(diags, df2_gmo)
  rownames(df2_gmo)[1] = "diag"
  df2_gmo = apply(df2_gmo, c(1,2), as.numeric)
  
  # PLS model w/ Walktrap feature selection
  df2_walktrap = data_mx[which(rownames(data_mx) %in% walktrap_module_select[[model]]), which(colnames(data_mx) %in% df_all$ptID)]
  df2_walktrap = rbind(df_all$bits, df2_walktrap)
  rownames(df2_walktrap)[1] = "metabolomics_evidence"
  df2_walktrap = rbind(diags, df2_walktrap)
  rownames(df2_walktrap)[1] = "diag"
  df2_walktrap = apply(df2_walktrap, c(1,2), as.numeric)
  
  dff_model = data.frame(pls_var=numeric(), pls_ctd=numeric(), pls_infomap=numeric(), 
                         pls_gmo=numeric(), pls_walktrap=numeric(),
                         pls_var_ctd=numeric(), pls_ctd_ctd=numeric(), pls_infomap_ctd=numeric(), 
  ...
  for (it in 1:ncol(df2)) {
    ...
    # Top Zscore-based feature selection: Train model
    ...
    # InfoMap feature selection: Train model
    df2_infomap_2 = df2_infomap[-which(rownames(df2_infomap)=="metabolomics_evidence"), ]
    infomap_res = plsr(diag~., data = as.data.frame(t(df2_infomap_2[,isTrain])))
    varImp_im[[it]] = varImp(infomap_res)
    tst_data = df2_infomap_2[-1,-isTrain]
    infomap_tst = predict(infomap_res, ncomp=infomap_res$ncomp, newdata=as.data.frame(t(tst_data)))
    dff_model[it, "pls_infomap"] = infomap_tst
    
    # GMO feature selection: Train model
    df2_gmo_2 = df2_gmo[-which(rownames(df2_gmo)=="metabolomics_evidence"), ]
    gmo_res = plsr(diag~., data = as.data.frame(t(df2_gmo_2[,isTrain])))
    varImp_gmo[[it]] = varImp(gmo_res)
    tst_data = df2_gmo_2[-1,-isTrain]
    gmo_tst = predict(gmo_res, ncomp=gmo_res$ncomp, newdata=as.data.frame(t(tst_data)))
    dff_model[it, "pls_gmo"] = gmo_tst
    
    # WalkTrap feature selection: Train model
    df2_walktrap_2 = df2_walktrap[-which(rownames(df2_walktrap)=="metabolomics_evidence"), ]
    walktrap_res = plsr(diag~., data = as.data.frame(t(df2_walktrap_2[,isTrain])))
    varImp_wt[[it]] = varImp(walktrap_res)
    tst_data = df2_walktrap_2[-1,-isTrain]
    walktrap_tst = predict(walktrap_res, ncomp=walktrap_res$ncomp, newdata=as.data.frame(t(tst_data)))
    dff_model[it, "pls_walktrap"] = walktrap_tst
  ...
  df_varImp = data.frame(varimp=c(apply(as.data.frame(varImp_z), 1, mean),
                                   apply(as.data.frame(varImp_ctd), 1, mean),
                                   apply(as.data.frame(varImp_im), 1, mean),
                                   apply(as.data.frame(varImp_gmo), 1, mean),
                                   apply(as.data.frame(varImp_wt), 1, mean)),
                          metabolite=c(zscore_module_select[[model]], ctd_module_select[[model]], 
                                       infomap_module_select[[model]], gmo_module_select[[model]],
                                       walktrap_module_select[[model]]),
                          model=c(rep("Zscore", length(zscore_module_select[[model]])),
                                  rep("CTD", length(ctd_module_select[[model]])),
                                  rep("FSFCN-InfoMap", length(infomap_module_select[[model]])),
                                  rep("FSFCN-GMO", length(gmo_module_select[[model]])),
                                  rep("FSFCN-WalkTrap", length(walktrap_module_select[[model]]))))
  ...
  df_varImp2 = data.frame(varimp=c(apply(as.data.frame(varImp_z_ctd), 1, mean),
                          ...
                          metabolite=c("CTD.cov", zscore_module_select[[model]], "CTD.cov", ctd_module_select[[model]], 
                                       "CTD.cov", infomap_module_select[[model]], "CTD.cov", gmo_module_select[[model]],
                                       "CTD.cov", walktrap_module_select[[model]]),
                          model=c(rep("Zscore", 1+length(zscore_module_select[[model]])),
                                  ...
                                  rep("FSFCN-WalkTrap", 1+length(walktrap_module_select[[model]]))))
  
  ggplot(df_varImp, aes(x=metabolite, y=varimp, group=model, colour=model)) + geom_line() 
  ...
  plsinfomap_auc = roc(diags, dff_model[,"pls_infomap"])
  plsgmo_auc = roc(diags, dff_model[,"pls_gmo"])
  plswalktrap_auc = roc(diags, dff_model[,"pls_walktrap"])
  ...
  print(sprintf("PLS.infomap     AUC = %.3f", plsinfomap_auc$auc))
  print(sprintf("PLS.gmo     AUC = %.3f", plsgmo_auc$auc))
  print(sprintf("PLS.walktrap     AUC = %.3f", plswalktrap_auc$auc))
  ...
  plsinfomap_auc2 = coords(plsinfomap_auc, "best", ret=c("threshold", "specificity", "accuracy", "precision", "recall"))
  plsgmo_auc2 = coords(plsgmo_auc, "best", ret=c("threshold", "specificity", "accuracy", "precision", "recall"))
  plswalktrap_auc2 = coords(plswalktrap_auc, "best", ret=c("threshold", "specificity", "accuracy", "precision", "recall"))
  ...
  print(plsvar_auc2)
  print(plsctd_auc2)
  print(plsinfomap_auc2)
  print(plsgmo_auc2)
  print(plswalktrap_auc2)
  ...
  df_vis[[model]] = data.frame(specificity=c(1-plsvar_auc$specificities, 1-plsctd_auc$specificities, 
                                             1-plsinfomap_auc$specificities, 1-plsgmo_auc$specificities, 
                                             1-plswalktrap_auc$specificities, 
                                             1-plsvar_ctd_auc$specificities, 1-plsctd_ctd_auc$specificities, 
                                             1-plsinfomap_ctd_auc$specificities, 1-plsgmo_ctd_auc$specificities,
                                             1-plswalktrap_ctd_auc$specificities, 1-ctd_auc$specificities),
                      sensitivity=c(plsvar_auc$sensitivities, plsctd_auc$sensitivities, 
                                             plsinfomap_auc$sensitivities, plsgmo_auc$sensitivities, 
                                             plswalktrap_auc$sensitivities, 
                                             plsvar_ctd_auc$sensitivities, plsctd_ctd_auc$sensitivities, 
                                             plsinfomap_ctd_auc$sensitivities, plsgmo_ctd_auc$sensitivities,
                                             plswalktrap_ctd_auc$sensitivities, ctd_auc$sensitivities),
                      method=c(rep("PLSvar", length(plsvar_auc$specificities)), 
                               rep("PLSctd", length(plsctd_auc$specificities)),
                               rep("PLSinfomap", length(plsctd_auc$specificities)),
                               rep("PLSgmo", length(plsctd_auc$specificities)),
                               rep("PLSwalktrap", length(plsctd_auc$specificities)),
                               rep("PLSvar-CTD", length(plsvar_ctd_auc$specificities)),
                               rep("PLSctd-CTD", length(plsvar_ctd_auc$specificities)),
                               rep("PLSinfomap-CTD", length(plsvar_ctd_auc$specificities)),
                               rep("PLSgmo-CTD", length(plsvar_ctd_auc$specificities)),
                               rep("PLSwalktrap-CTD", length(plsvar_ctd_auc$specificities)),
                               rep("CTD", length(ctd_auc$specificities))))
}

cit_mets = c("1-stearoyl-gpi (18:0)", "2-hydroxybutyrate (ahb)", "2-stearoylglycerophosphoinositol", "3-methyl-2-oxovalerate", "3-ureidopropionate", "4-methyl-2-oxopentanoate", "betaine", "citrulline", "deoxycarnitine", "gamma-glutamylleucine", "leucine", "n-acetylalanine", "phenylacetylglutamine", "stearate, methyl ester", "stearidonate (18:4n3)", "tartarate", "tryptophan", "uridine")
gamt_mets = c("3-(4-hydroxyphenyl)lactate (hpla)", "creatine", "erythritol", "linolenate (18:3n3 or 3n6)", "gamma-glutamylleucine", "glycocholenate sulfate", "n-acetyl-beta-alanine", "tartarate")
msud_mets = c("1-linolenoylglycerophosphocholine (18:3n3)", "2-aminoheptanoate", "2-methylbutyroylcarnitine (c5)", "2-myristoyl-gpc (14:0)", "3-hydroxydecanoate", "3-hydroxyisobutyrate", "3-methyl-2-oxovalerate", "4-androsten-3beta,17beta-diol disulfate 1", "4-methyl-2-oxopentanoate", "allo-isoleucine", "alpha-hydroxyisovalerate", "homostachydrine", "hydroxyisovaleroylcarnitine (c5)", "isoleucine", "isovalerate (c5)", "isovalerylcarnitine (c5)", "pyroglutamine", "tartarate", "tiglyl carnitine (c5)")
mma_mets = c("1-margaroylglycerophosphoethanolamine", "2-hydroxypalmitate", "2-methylbutyroylcarnitine (c5)", "2-methylmalonyl carnitine", "3-hydroxypropanoate", "carnitine", "deoxycarnitine", "gamma-glutamyl-2-aminobutyrate", "glycine", "homostachydrine", "hydroxyisovaleroylcarnitine (c5)", "imidazole lactate", "leucine", "margarate (17:0)", "n-acetylthreonine", "pentadecanoate (15:0)", "propionylcarnitine (c3)", "succinylcarnitine (c4)", "tiglyl carnitine (c5)", "uridine", "valerylcarnitine (c5)")
pa_mets = c("1-margaroylglycerophosphoethanolamine", "1-pentadecanoylglycerophosphocholine (15:0)", "2-methylmalonyl carnitine", "3-hydroxypropanoate", "3-methylglutaroylcarnitine (c6)", "3-methyl-2-oxobutyrate", "5-dodecenoate (12:1n7)", "allo-isoleucine", "beta-hydroxyisovalerate", "carnitine", "cysteine", "deoxycarnitine", "ergothioneine", "fumarate", "gamma-glutamylisoleucine", "gamma-glutamylvaline", "glutaroylcarnitine (c5)", "glycine", "hydroxyisovaleroylcarnitine (c5)", "pentadecanoate (15:0)", "phenylcarnitine", "propionylcarnitine (c3)", "succinylcarnitine (c4)", "tiglyl carnitine (c5)", "uridine", "valerylcarnitine (c5)", "valine")
pku_mets = c("1-eicosatrienoylglycerophosphoethanolamine", "3-hydroxyisobutyrate", "7-hoca", "bilirubin", "biliverdin", "gamma-glutamylphenylalanine", "glycolate (hydroxyacetate)", "n-acetylaspartate (naa)", "n-acetylphenylalanine", "nicotinamide", "phenylalanine", "phenylalanylisoleucine", "phenyllactate (pla)", "tryptophylphenylalanine")
otc_mets = c("3-methyl-2-oxobutyrate", "3-methyl-2-oxovalerate", "4-methyl-2-oxopentanoate", "phenylacetylglutamine", "stearidonate (18:4n3)", "uridine", "glutamine", "isovalerate (c5)", "1-stearoyl-gpc (18:0)", "tryptophan")
gamt_mets = c("gamma-glutamylleucine", "glycocholenate sulfate", "linolenate (18:3n3 or 3n6)", "n-acetyl-beta-alanine", "creatine", "tartarate", "3-(4-hydroxyphenyl)lactate (hpla)", "erythritol", "gamma-glutamylisoleucine", "gamma-glutamylleucine", "gamma-glutamyllysine", "gamma-glutamylphenylalanine", "gamma-glutamylthreonine", "gamma-glutamylvaline")

pku_mets[!(pku_mets %in% rownames(data_mx))]
sapply(pku_mets[!(pku_mets %in% rownames(data_mx))], function(i) rownames(data_mx)[grep(i, rownames(data_mx))])
...
discrimination = list()
for (model in c("cit", "msud", "mma", "pa", "pku")) {
  i = which(c("cit", "gamt", "msud", "mma", "pa", "pku")==model)
  select_mets = list(cit_mets, gamt_mets, msud_mets, mma_mets, pa_mets, pku_mets)[[i]]
...
dd = data.frame(zscore=as.numeric(Miller2015["3-(4-hydroxyphenyl)lactate (hpla)", grep("IEM_", colnames(Miller2015))]),
                diags = diags, stringsAsFactors = FALSE)
ggplot(dd) + geom_boxplot(aes(x=diags, y=zscore))
