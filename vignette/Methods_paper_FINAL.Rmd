...
``` {r pathway-vis}
# ABAT patients
#plot.pathwayMap("allPathways", "GABAT001", data_mx[,"GABAT001"], 1, 1, getwd())
#plot.pathwayMap("allPathways", "GABAT002", data_mx[,"GABAT002"], 1, 1, getwd())
#plot.pathwayMap("allPathways", "GABAT003", data_mx[,"GABAT003"], 1, 1, getwd())
#plot.pathwayMap("allPathways", "GABAT004", data_mx[,"GABAT004"], 1, 1, getwd())
#plot.pathwayMap("allPathways", "GABAT005", data_mx[,"GABAT005"], 1, 1, getwd())
#plot.pathwayMap("allPathways", "GABAT006", data_mx[,"GABAT006"], 1, 1, getwd())
#plot.pathwayMap("allPathways", "GABAT007", data_mx[,"GABAT007"], 1, 1, getwd())
# AADC patients
...
#plot.pathwayMap("TCA-Cycle", "GABAT001", abat_data[,"GABAT001"], 1, 2, getwd())
# For PNG (not SVG)
#plot.pathwayMap("TCA-Cycle", "GABAT001", abat_data[,"GABAT001"], 1, 2, getwd(), FALSE)
#plot.pathwayMap("Asp-Glu-Metabolism", "GABAT001", abat_data[,"GABAT001"], 1, 2, getwd(), FALSE)
#plot.pathwayMap("FattyAcid-Metabolism", "GABAT001", data_mx[,"GABAT001"], 1, 1, getwd(), FALSE)
```
...
stats.getORA_Metabolon(data_mx[,"GABAT001"], threshold = 2, type = "zscore")
stats.getORA_Metabolon(data_mx[,"GABAT002"], threshold = 2, type = "zscore")
stats.getORA_Metabolon(data_mx[,"GABAT003"], threshold = 2, type = "zscore")
stats.getORA_Metabolon(data_mx[,"GABAT004"], threshold = 2, type = "zscore") ## This one
stats.getORA_Metabolon(data_mx[,"GABAT005"], threshold = 2, type = "zscore")
stats.getORA_Metabolon(data_mx[,"GABAT006"], threshold = 2, type = "zscore")
stats.getORA_Metabolon(data_mx[,"GABAT007"], threshold = 2, type = "zscore")
...
require(CTD)
for (model in c("aadc", "abat", "adsl")) {
  load(file.path(normalizePath(op_path),sprintf("Clinical_paper/bg_%s_maps.RData",model)))
  ig_pruned = graph.naivePruning(ig, ig_ref)
  adj_mat = as.matrix(get.adjacency(ig_pruned, attr="weight"))
  write.table(adj_mat, 
              file=file.path(normalizePath(op_path),sprintf("graphs/graph%s_glasso_maps_naive.txt",toupper(model))), 
              sep="\t",
              col.names=TRUE,
              row.names=TRUE)
  save(ig_pruned, file=file.path(normalizePath(op_path),sprintf("graphs/bg_%s_maps_naive.RData",model)))
}

```
## Interpret Patient Metabolomics Profiles using CTD and Disease-Specific Networks
``` {r multinode-model-allpts-without-X-compounds}
...
dir.create(file.path(getwd(), "MultiNode"), showWarnings = FALSE)
...
for (p in 1:ncol(data_mx)) {
  ...
  save(ptBSbyK, df, file=sprintf("MultiNode/mn_model_%s_maps.RData", model))
}
...
# FILE HIERARCHY: 
#        graphs/ folder with all graph%s_glasso_maps_naive.txt files in it.
#        wrapper_ranks.r
#        getRanks.pbs
#        getRanksN.r
# DIRECTIONS: Run wrapper_ranks.r on cluster with for each pruned disease network. Set type="naive" since we want the 
...
#---- generate graph%s_glasso_maps_naive.txt files ----

setwd('./SingleNodeRankings/graphs')
for (model in c('aadc','abat','adsl')){
  ig = loadToEnv(system.file('networks', sprintf('bg_%s_maps_naive.RData',model), package='CTD'))[['ig_pruned']]
  adj_mat = as.matrix(get.adjacency(ig, attr="weight"))
  write.table(adj_mat, 
              file=sprintf("./graph%s_glasso_maps_naive.txt",toupper(model))), 
              sep="\t",
              col.names=TRUE,
              row.names=TRUE)
}

#---- define function singleNodeDiffusion.getRanksN ----
singleNodeDiffusion.getRanksN = function (n, G) {
  all_nodes = names(G)
  ...
  while (stopIterating == FALSE) {
    print(length(current_node_set))
    currentGraph[1:length(currentGraph)] = 0
    ...
    if (abs(p1_event - 1) > thresholdDiff) {
      ...
      currentGraph[!(names(currentGraph) %in% names(current_node_set))] = unlist(currentGraph[!(names(currentGraph) %in% 
                names(current_node_set))]) + extra.prob.to.diffuse/sum(!(names(currentGraph) %in% names(current_node_set)))
    }
    maxProb = names(which.max(currentGraph))
    startNode = names(currentGraph[maxProb[1]])
    current_node_set = c(current_node_set, startNode)
    if (length(c(startNode, current_node_set)) >= (length(G))) {stopIterating = TRUE}
  }
...
args = commandArgs(trailingOnly=TRUE)
diag = args[1]
type = args[2] # "D" or "REF" or "naive"

require(igraph)
output_dir = sprintf("./diag%s%s", diag, type)
str = sprintf("mkdir %s", output_dir)
system(str) 

if (type=="REF") {
  tmp = as.matrix(read.table(sprintf("./graphs/graph%s%s_glasso_maps.txt", diag, type), sep="\t", header=TRUE))
} else if (type=="D") {
  tmp = as.matrix(read.table(sprintf("./graphs/graph%s_glasso_maps.txt", diag), sep="\t", header=TRUE))
} else {
  tmp = as.matrix(read.table(sprintf("./graphs/graph%s_glasso_maps_naive.txt", diag), sep="\t", header=TRUE))
}
...
for (n in 1:totalN) {
  str = sprintf("qsub -v diag=%s,type=%s,n=%d getRanks.pbs", diag, type, n)
...
last_sum = 0
while (!ready) {
  f = list.files(sprintf("./diag%s%s", diag, type), pattern=".RData")
...
all_nodes = rownames(tmp)
permutationByStartNode = list()
for (n in 1:length(all_nodes)) {
  load(sprintf("./diag%s%s/%d.RData", diag, type, n))
  permutationByStartNode[[n]] = toupper(current_node_set)
}
names(permutationByStartNode) = all_nodes
save.image(file=sprintf("./diag%s%s/%s%s-ranks.RData", diag, type, diag, type))
print("collate complete...")
#str = sprintf("rm -r ./diag%s/%s", diag, type)
#system(str)
...
args = commandArgs(trailingOnly=TRUE)
diag = args[1]
type = args[2] # either "D" or "REF" or "naive"
n = as.numeric(args[3])
...
require(CTD)
source("singleNodeDiffusion.getRanksN.r")

if (type=="REF") {
  tmp = as.matrix(read.table(sprintf("../graphs/graph%s%s_glasso_maps.txt", diag, type), sep="\t", header=TRUE, check.names=FALSE))
} else if (type=="D") {
    tmp = as.matrix(read.table(sprintf("../graphs/graph%s_glasso_maps.txt", diag), sep="\t", header=TRUE, check.names=FALSE))
} else {
  tmp = as.matrix(read.table(sprintf("../graphs/graph%s_glasso_maps_naive.txt", diag), sep="\t", header=TRUE, check.names=FALSE))
}
rownames(tmp) = colnames(tmp)
dim(tmp)
...
current_node_set = singleNodeDiffusion.getRanksN(n, G)
new_dir =  sprintf("./diag%s%s", diag, type)
if (!dir.exists(new_dir)) {
  str = sprintf("mkdir %s", new_dir)
  system(str)
}
save(current_node_set, file=sprintf("./diag%s%s/%d.RData", diag, type, n))
...
#    
#    module load R/3.3
#    cd /home/Xiqi.Li2/Rep_Method_Paper/CTD_Method/v2/SingleNodeRankings
#    
#    diag=${diag}
#    type=${type}
#    n=${n}
#    Rscript getRanksN.r $diag $type $n > ranks:$diag$type-$n.out
#    rm ranks:$diag$type-$n.out


```

``` {r single-node}
# Once you have the pre-computed node ranks (using single-node diffusion) from cluster, get the metabolomics evidence for each disease network context using the single-node encoding algorithm.

require(R.utils)
require(CTD)

dir.create(file.path(getwd (), "SingleNode"), showWarnings = FALSE)
...
for (model in c("aadc", "abat", "adsl")) {
  ...
  G = vector(mode="list", length=length(V(ig)$name))
  names(G) = V(ig)$name
  data(Thistlethwaite2019) # needs to be reset every loop
  data_mx = as.matrix(data_mx)
  data_mx = suppressWarnings(apply(data_mx, c(1,2), as.numeric))
  data_mx = data_mx[,-c(1,2,3,4,5,6,7,8)]
  data_mx = data_mx[which(rownames(data_mx) %in% V(ig)$name),]
  ...
  ranks = loadToEnv(sprintf("SingleNodeRankings/%snaive-ranks.RData", toupper(model)))[["permutationByStartNode"]]
  ...
  for (p in 1:ncol(data_mx)) {
    ...
    save(ptBSbyK, df, file=sprintf("./SingleNode/sn_model_%s_maps.RData", model))
  }
}

```

## Supp Table S2 Single-Node vs. Multi-Node Signal in 3 Example Patients
``` {r singleNode-nof1}
...
png("/Users/lillian.rosa/OneDrive/MacFiles/9thCommitteeMeeting/Methods_paper/nof1/GABAT004_module.png")
plot.igraph(e, layout=layout.circle, edge.width=50*abs(E(e)$weight))
dev.off()
svg("/Users/lillian.rosa/OneDrive/MacFiles/9thCommitteeMeeting/Methods_paper/nof1/GABAT004_module.svg")
plot.igraph(e, layout=layout.circle, edge.width=50*abs(E(e)$weight))
dev.off()
...
## Figure 5: Illustration of CTD's specificity for AADC, ABAT, ADSL models.
``` {r multinode-model-allpts-without-X-compounds local_version}
setwd("/Users/lillian.rosa/OneDrive/MacFiles/9thCommitteeMeeting/Methods_paper/")
rm(list=ls())
require(R.utils)
require(CTD)
...
for (model in c("aadc", "abat","adsl")) {
  ...
  G = vector(mode="list", length=length(V(ig)$name))
  names(G) = V(ig)$name
  data(Thistlethwaite2019) # needs to be reset every time
  data_mx = as.matrix(data_mx)
  data_mx = suppressWarnings(apply(data_mx, c(1,2), as.numeric))
  data_mx = data_mx[,-c(1,2,3,4,5,6,7,8)]
  data_mx = data_mx[which(rownames(data_mx) %in% V(ig)$name), ]
...
# Figure 5: Plot patient ID vs. bits for each graph, for AADC, ABAT and then ADSL
require(ggplot2)
p=list()
for (model in c('aadc','abat','adsl')){
  load(sprintf('./MultiNode/mn_model_%s_maps.RData',model))
  pts = unique(df$ptID)
...
model='aadc'
png(sprintf("./results/%s_bg_naive.png", model), height=1000, width=1500, res=350)
  p[[model]]
dev.off()
...
model='aadc'
png(sprintf("./ctd_pls/ctd_pls_%s.png", model))
ggplot(df_vis[[model]]) + geom_line(aes(x=specificity, y=sensitivity, colour=method), size=3) +
...
dir.create("./ptSim", showWarnings = TRUE)
#### Look for patients, referece and udn cases that show significance ####
for (corr in c(TRUE,FALSE)){
  for (mem in c('sn','mn')){
    dir_name = if (mem == 'sn') 'SingleNode' else 'MultiNode'
    udns_matches = list()
    for (model in c('aadc','abat','adsl')){
      load(sprintf("%s/%s_model_%s_maps.RData",dir_name,mem,model))
      ...
      pt_sig = df_memory[which(df_memory$m>4.32), "ptID"]
      pt_sig2 = pt_sig[grep("UDN", pt_sig)]
      bm_df = df_memory[which(df_memory$ptID %in% pt_sig2),]
      sprintf('diffusion:%s; model:%s; Bonferoni Correction:%i',mem,model,corr)
      dim(bm_df)
      bm_df[grep("succinamic acid|2-pyrrolidinone|n6-succinyladenosine|3-methoxytyrosine", bm_df$S),] #optional
      pt_sig2_bm=bm_df[
        grep("succinamic acid|2-pyrrolidinone|n6-succinyladenosine|3-methoxytyrosine", bm_df$S), "ptID"]
      udns_matches[[model]] = pt_sig
      udns_matches[["bm"]] = unique(c(udns_matches[["bm"]],pt_sig2_bm))
      save(udns_matches, file=sprintf("ptSim/%s_udns_cor%i_matches.RData",mem,corr))
    }
  }
}
...
for (mem in c('sn','mn')){
  for (corr in c(TRUE,FALSE)){
      if (corr == TRUE){
        load(sprintf("ptSim/%s_udns_cor%i_matches.RData",mem,corr))
        udn_bm = c(udn_bm,udns_matches$bm)
        refs_no_hits=unique(c(refs_no_hits,unlist(udns_matches)[grep("REF", unlist(udns_matches))]))
      }else{
        load(sprintf("ptSim/%s_udns_cor%i_matches.RData",mem,corr))
        refs_no_hits=unique(c(refs_no_hits,unlist(udns_matches)[grep("REF", unlist(udns_matches))]))
      }
...
for (model in c("aadc", "abat", "adsl")) {
  ...
  save.image(sprintf("./ptSim/sn_ptSim_%s.RData", model))
  ...
  save.image(sprintf("./ptSim/sn_ptSim_%s_norm.RData", model))
}
...
for (alpha in c(1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0)) {
  ...
  for (model in c('aadc','abat','adsl')){
    res_all = loadToEnv(sprintf("./ptSim/sn_ptSim_%s_norm.RData", "abat"))[["res_all"]]
  ...
  orca(ctd_p, sprintf("ncd_2D_%d.png", 100*alpha))
...
dir.create("./ptSim/figure", showWarnings = TRUE)
system('mv ./ncd_*.html ./ptSim/figure')
...
data_mx = data_mx[,-c(1,2,3,4,5,6,7,8)]
ref_data = data_mx[, grep("REF", colnames(data_mx))]
load("./ptSim/sn_ptSim_aadc_norm.RData")
...
orca(pear_p, "./ptSim/figure/pearson_v2.png")
...
orca(mx_p, "./ptSim/figure/mx_v2.png")
...
orca(euc_p, "./ptSim/figure/euc_v2.png")
...
orca(man_p, "./ptSim/figure/man_v2.png")
