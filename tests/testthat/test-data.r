...
test_that("Surrogate profiles works", {
  ...
  expect_equal(any(grepl("disease_surr", colnames(surr.data))) && any(grepl("control_surr", colnames(surr.data))), TRUE)
})
...
test_that("Probability diffusion works", {
  ...
  G_new = graph.diffuseP1(p1=1.0, startNode="A", G=G, visitedNodes="A", 
  ...
  expect_equal(G_new[which.max(G_new)], )
  ...
  G_new = graph.diffuseP1(p1=1.0, startNode="A", G=G, visitedNodes=c("A","B","C"), 
...
test_that("Multi-node pipeline works", {
  ...
  ranks = multiNode.getNodeRanks(S=S, G=G, p1=0.9, thresholdDiff=0.01, adj_mat=adj_mat,
                                 ...
                                 output_dir=getwd(), useLabels=TRUE, coords=coords)
...
test_that("Patient distances works", {
  ...
  for (pt in seq_len(ncol(data_mx))) {
   ...
   ptBSbyK[[colnames(data_mx)[pt]]] = singleNode.getPtBSbyK(S, ranks)
  }
