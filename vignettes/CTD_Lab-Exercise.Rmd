...
G_new = graph.diffuseP1(p1, startNode, G, visitedNodes, thresholdDiff, adj_mat, TRUE, output_dir = sprintf("%s/images", getwd()), 1, coords)
# Inherited probabilities across all nodes should add to 1.
sum(unlist(G_new))
# Which node inherited the highest probability from startNode?
G_new[which.max(G_new)]
...
ranks = multiNode.getNodeRanks(S = c("A", "B"), G, p1, thresholdDiff, adj_mat, log2(length(G)), FALSE, output_dir = sprintf("%s/images", getwd()), TRUE, coords)

# Get node ranks as list object, with no images generated
ranks = multiNode.getNodeRanks(S = c("A", "B"), G, p1, thresholdDiff, adj_mat, log2(length(G)), FALSE)
```
...
for (n in seq_len(length(S))) {
  ...
  ranks[[n]] = singleNode.getNodeRanksN(ind, G, p1, thresholdDiff, adj_mat, S = c("A", "B"), 
...
for (n in 1:length(S_arg)) {
  ...
  ranks[[n]] = singleNode.getNodeRanksN(ind, G, p1, thresholdDiff, adj_mat, S = names(S_arg), 
...
```{r patient_distances, eval=FALSE}
...
for (n in 1:length(S_arg)) {
  ...
  ranks[[n]] = singleNode.getNodeRanksN(ind, G, p1, thresholdDiff, adj_mat, S = S_arg, 
                                        num.misses = log2(length(G)), TRUE)
}
names(ranks) = S_arg
...
for (pt in 1:ncol(data_mx)) {
  ...
  for (pt2 in pt:ncol(data_mx)) {
    ...
    tmp = mle.getPtDist(ptBSbyK[[ptID]], ptID, ptBSbyK[[ptID2]], ptID2, data_mx, ranks, p1, thresholdDiff, adj_mat)
    for (k in 1:kmx) {
...
names(dd) = colnames(res_all[[1]]$ncd)

require(plotly)
require(gplots)
require(RColorBrewer)
colnames(ncd) = colnames(res_all[[1]]$ncd)
rownames(ncd) = colnames(res_all[[1]]$ncd)
p = plot.mdsDist(ncd, dd, k=2, NULL)
p 
# Hierarchical clustering
# A PNG image called "ptDistances.png" will save in the path set as parameter "path" in plot.hmDist function call.
