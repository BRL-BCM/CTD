...
#' @param data - Data matrix with observations (e.g., patient samples) as columns, features 
#'               (e.g., metabolites or genes) as rows
#' @param std - The level of variability (standard deviation) around each observed feature's
#'             z-score you want to add to generate the surrogate profiles.
#' @param ref_data - Data matrix for healthy control "reference" samples, observations 
#'                  (e.g., patient samples) as columns, features (e.g., metabolites or genes) 
#'                  as rows
