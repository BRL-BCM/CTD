...
mle.getPermMovie_memoryless = function(subset.nodes, ig, output_filepath, movie=TRUE, subset=FALSE) {
  if (subset) {
  ...
  igraphObjectG = lapply(igraphObjectG, function(i) i[[1]]=0)
  ...
  if (movie==TRUE) {
    ...
    plot.igraph(ig.subset, layout=coords, vertex.color=V(ig.subset)$color, edge.arrow.size=0.01,
    ...
    while (stopIterating==FALSE) {
      ...
      if (movie==TRUE) {
        ...
        plot.igraph(ig.subset, layout=coords, vertex.color=V(ig.subset)$color,
                    vertex.label=V(ig.subset)$label, edge.width=20*abs(E(ig.subset)$weight), edge.arrow.size=0.01,
...
mle.getPermMovie_memory = function(subset.nodes, ig, output_filepath, movie=TRUE, subset=FALSE) {
  ...
  if (movie==TRUE) {
    ...
    png(sprintf("%s/diffusionP1Movie%d.png", output_filepath, length(current_node_set)), 500, 500)
    plot.igraph(ig.subset, layout=coords, vertex.color=V(ig.subset)$color, edge.arrow.size=0.01,
                vertex.label=V(ig.subset)$label, edge.width=abs(E(ig.subset)$weight),  mark.col="dark red",
                mark.groups = current_node_set)
    title(sprintf("{%s}", paste(as.numeric(current_node_set %in% subset.nodes), collapse="")), cex.main=2)
    legend("bottomleft", legend=c("Jackpot Nodes", "Drawn Nodes"), fill=c("green", "dark red"))
    dev.off()
  ...
  for (n in 1:length(subset.nodes)) {
    ...
    while (stopIterating==FALSE) {
      current_node_set = c(current_node_set, startNode)
      sumHits = as.vector(matrix(0, nrow=length(V(ig.subset)$name), ncol=1))
      names(sumHits) = names(igraphObjectG)
      ...
      if (movie==TRUE) {
        png(sprintf("%s/diffusionP1Movie%d_%d.png", output_filepath, n, length(current_node_set)), 500, 500)
        plot.igraph(ig.subset, layout=coords, vertex.color=V(ig.subset)$color,
                    vertex.label=V(ig.subset)$label, edge.width=abs(E(ig.subset)$weight), edge.arrow.size=0.01,
                    vertex.size=5+round(50*unlist(currentGraph), 0), mark.col="dark red",
                    mark.groups = current_node_set)
        title(sprintf("{%s}", paste(as.numeric(current_node_set %in% subset.nodes), collapse="")), cex.main=2)
        legend("bottomleft", legend=c("Jackpot Nodes", "Drawn Nodes"), fill=c("green", "dark red"))
