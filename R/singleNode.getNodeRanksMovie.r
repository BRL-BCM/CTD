...
#' singleNode.getNodeRanksMovie(subset.nodes, ig, output_filepath = getwd(), movie=TRUE)
singleNode.getNodeRanksMovie = function(subset.nodes, ig, output_filepath, movie=TRUE, zoomIn=FALSE) {
  if (zoomIn) {
  ...
  # Phase 1: Do node rankings ahead of time for each possible startNode in subGraphS.
  if (movie==TRUE) {
