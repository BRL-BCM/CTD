...
def get_encoding_length(bs, G, pvals=None, pt_id=None, pen_p_val=False):
    ...
    for k in range(len(bs)):
        ...
        not_found = k - found + 1
        if pen_p_val:
            e = np.float64(np.log2(np.float128(math.comb(len(G), (not_found + 1))))) + len(opt_bs) - 1 # encoding length
        else:
        ...
        row['opt.T'] = found
        if pen_p_val:
            row['IS.null'] = np.float64(np.log2(np.float128(math.comb(len(G), (k + 1)))))
        else:
            row['IS.null'] = np.float64(np.log2(len(G)) * (k + 1))  # Shannon's entropy
        row['IS.alt'] = e
        row['d.score'] = round(row['IS.null'] - row['IS.alt'], 3)
