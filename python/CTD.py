...
if os.path.exists(argv.adj_matrix):
    ...
    adj_df = pd.DataFrame(adj_df, index=experimental_df.index,
                          columns=experimental_df.index)  # keep as DataFrame for now
...
for node in S_perturbed_nodes: # TODO: CONSIDER ADDING tqdm
